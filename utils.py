...
async def upload_to_supabase(file: UploadFile, file_path: str) -> str:
    """Upload file to Supabase storage using a thread-safe background task."""
    try:
        ...
        return await asyncio.get_event_loop().run_in_executor(executor, _upload)
...
async def delete_from_supabase(file_path: str) -> bool:
    ...
    try:
        def _delete():
            return supabase.storage.from_(settings.SUPABASE_STORAGE_BUCKET).remove([file_path])
        await asyncio.get_event_loop().run_in_executor(executor, _delete)
        return True
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def upload_news_image(file: UploadFile) -> str:
    ...
    image_url = await upload_to_supabase(file, file_path)
    print("🟢 Upload complete. Image URL:", image_url)

    if not image_url:
        raise HTTPException(status_code=500, detail="Image upload returned empty URL")
    return image_url
...
async def upload_profile_image(file: UploadFile, user_id: str) -> str:
    """Upload profile image with organized path structure"""
    if not file.content_type.startswith('image/'):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Only image files are allowed"
    ...
    file_ext = Path(file.filename).suffix.lower()
    file_path = f"profiles/{user_id}{file_ext}"

    print("🟡 Uploading profile image for user:", user_id)
    image_url = await upload_to_supabase(file, file_path)
    print("🟢 Upload complete. Image URL:", image_url)

    if not image_url:
        raise HTTPException(status_code=500, detail="Profile image upload returned empty URL")
    return image_url